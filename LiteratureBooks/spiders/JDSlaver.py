...
class JdSpider(RedisCrawlSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bloomfilter = BloomFilter(key = 'jd_bloomfilter')
    def parse(self, response):
        ...
        if search_result_num != 0:
            try:
                ...
                if not self.bloomfilter.is_contains(item["book_url_jd"]+item["book_price_jd"]):
                    yield item
                    self.bloomfilter.insert(item["book_url_jd"]+item["book_price_jd"])
            except Exception,e:
